...
export default async function Page({
  ...
  const { rows, totalPages, page: safePage } = await listResourceRowsPaged<Procedure>("procedures", {
    onlyActive: true,
    search: q,
    searchColumns: ["name"],
    orderBy: "name",
  ...
  function buildHref(targetPage: number) {
    ...
    return `/contenido/procedimientos?${params.toString()}`;
  }
  ...
  return (
    <div className="flex min-h-dvh flex-col bg-bg">
      ...
      <main className="mx-auto w-full max-w-7xl flex-1 px-4 py-10 sm:px-6 lg:px-8">
        ...
        />
        <h1 className="mt-3 text-2xl font-semibold text-text">Procedimientos</h1>
            ...
            <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {rows.map((item) => (
                <SimpleContentCard
                  ...
                  title={item.name}
                  subtitle={item.sample || undefined}
