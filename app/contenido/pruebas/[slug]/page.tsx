      ...
      <main className="mx-auto w-full max-w-3xl flex-1 px-4 py-10 sm:px-6 lg:px-8">
        <Breadcrumbs
          items={[
            ...
            { label: "Pruebas", href: "/contenido/pruebas" },
            { label: item.name },
          ]}
        />
        ...
        <div className="mt-4">
          <AskLabdexAiButton sourceType="test" slug={item.slug} title={item.name} />
        </div>

        {item.description && <p className="mt-4 text-text-muted">{item.description}</p>}

        <div className="mt-6">
          <ResourceDetailSections resourceKey="laboratory_tests" record={item as unknown as Record<string, unknown>} />
        </div>
