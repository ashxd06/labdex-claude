...
export default async function Page({
  ...
  const { rows, totalPages, page: safePage } = await listResourceRowsPaged<LaboratoryTest>("laboratory_tests", {
    onlyActive: true,
    search: q,
    searchColumns: ["name"],
    orderBy: "name",
  ...
  function buildHref(targetPage: number) {
    ...
    return `/contenido/pruebas?${params.toString()}`;
  }
  ...
  return (
    <div className="flex min-h-dvh flex-col bg-bg">
      ...
      <main className="mx-auto w-full max-w-7xl flex-1 px-4 py-10 sm:px-6 lg:px-8">
        ...
        />
        <h1 className="mt-3 text-2xl font-semibold text-text">Pruebas</h1>
