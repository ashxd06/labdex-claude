  ...
  return (
    <div className="min-h-dvh bg-bg">
      ...
      <main className="mx-auto max-w-4xl px-4 py-14 sm:px-6 lg:px-8">
        <h1 className="text-2xl font-semibold uppercase tracking-wide text-text">
          {category.name}
        </h1>
        ...
        {!hasContent ? (
          ...
          <div className="mt-8 flex flex-col gap-8">
            {analyses.length > 0 && (
              <ContentGroup title="Análisis clínicos" items={analyses.map((a) => ({ name: a.name, sample: a.is_sample_data }))} />
            )}
            {tests.length > 0 && (
              <ContentGroup title="Pruebas de laboratorio" items={tests.map((t) => ({ name: t.name, sample: t.is_sample_data }))} />
            )}
            {procedures.length > 0 && (
              <ContentGroup title="Procedimientos" items={procedures.map((p) => ({ name: p.name, sample: p.is_sample_data }))} />
            )}
          </div>
        )}
      </main>
...
function ContentGroup({ title, items }: { title: string; items: { name: string; sample: boolean }[] }) {
  return (
    <div>
      ...
      <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-2">
        {items.map((item) => (
          <div
            key={item.name}
            className="flex items-center justify-between gap-2 rounded-md border border-border bg-surface px-4 py-3 text-sm text-text"
          >
            {item.name}
