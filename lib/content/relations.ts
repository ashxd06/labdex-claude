...
export async function getRelationsData(microorganismId: string) {
  ...
  const [allMedia, allTests, allProcedures, linkedMediaRaw, linkedTestsRaw, linkedProceduresRaw] =
    await Promise.all([
      supabase.from("culture_media").select("id, name").order("name") as unknown as Promise<{
        data: CultureMedia[] | null;
      }>,
      supabase.from("laboratory_tests").select("id, name").order("name") as unknown as Promise<{
        data: LaboratoryTest[] | null;
      }>,
      supabase.from("procedures").select("id, name").order("name") as unknown as Promise<{
        data: Procedure[] | null;
      }>,
      supabase
        .from("microorganism_media")
        .select("media_id, notes, culture_media(id, name)")
        .eq("microorganism_id", microorganismId),
      supabase
        .from("microorganism_tests")
        .select("test_id, result_expected, notes, laboratory_tests(id, name)")
        .eq("microorganism_id", microorganismId),
      supabase
        .from("microorganism_procedures")
        .select("procedure_id, notes, procedures(id, name)")
        .eq("microorganism_id", microorganismId),
    ]);

  const linkedMedia: LinkedMedia[] = (linkedMediaRaw.data ?? []).map(
    (row: { media_id: string; notes: string | null; culture_media: { id: string; name: string }[] | { id: string; name: string } | null }) => {
      const media = Array.isArray(row.culture_media) ? row.culture_media[0] : row.culture_media;
      return { id: row.media_id, name: media?.name ?? "—", notes: row.notes };
    }
  );
  ...
  const linkedTests: LinkedTest[] = (linkedTestsRaw.data ?? []).map(
    (row: {
      ...
      result_expected: string | null;
      notes: string | null;
      laboratory_tests: { id: string; name: string }[] | { id: string; name: string } | null;
    }) => {
  ...
  const linkedProcedures: LinkedProcedure[] = (linkedProceduresRaw.data ?? []).map(
    (row: { procedure_id: string; notes: string | null; procedures: { id: string; name: string }[] | { id: string; name: string } | null }) => {
      const procedure = Array.isArray(row.procedures) ? row.procedures[0] : row.procedures;
      return { id: row.procedure_id, name: procedure?.name ?? "—", notes: row.notes };
    }
  );
