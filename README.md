...
Continuación de las Fases 1, 2 y 3. Esta fase agrega el módulo profesional
de **Laboratorio Clínico**: pacientes, muestras, solicitudes de análisis,
registro de resultados (cuantitativos, cualitativos, semicuantitativos y
descriptivos), generación de informes con numeración atómica y exportación
a PDF con logo/firma/sello configurables.

## 1. Configurar Supabase

Ejecuta en el SQL Editor, en este orden (si ya ejecutaste `schema.sql` y
`schema_fase2.sql` en fases anteriores, sáltalos):

1. `supabase/schema.sql` (Fase 1)
2. `supabase/schema_fase2.sql` (Fase 2)
3. `supabase/schema_fase4.sql` (Fase 4 — nuevo)
4. *(Opcional)* `supabase/seed_lab_demo.sql` — crea un paciente marcado `is_demo = true` para probar el flujo completo.

En **Storage**, este script crea automáticamente el bucket **privado**
`lab-assets` (logo, firma, sello). No necesitas crearlo a mano.

## 2. Modelo de permisos de esta fase

Los datos de laboratorio son sensibles y **nunca son de lectura pública**:

- `patients`, `samples`, `lab_orders`, `lab_order_items`, `lab_results`, `lab_reports`: cualquier usuario **autenticado** (con sesión iniciada) puede leer y escribir. No se introdujo un tercer rol en esta fase; `profiles.role` queda preparado para uno (ej. `lab_staff`) sin migrar datos si se necesita más adelante.
- `lab_settings` y `reference_ranges`: cualquier autenticado puede leer (los necesita para trabajar), pero solo un `admin` puede modificarlos.
- El bucket `lab-assets` es **privado**: se sirve con signed URLs de 10 minutos (`getSignedLabAssetUrl`), nunca con una URL pública fija.
- El buscador público de LABDEX (`/buscar`, `/api/search`) **no cambió**: sigue consultando únicamente `categories`, `microorganisms`, `culture_media`, `laboratory_tests`, `procedures`, `clinical_analyses` y `documents`. Ninguna tabla de esta fase es alcanzable desde ahí.
- `/laboratorio/*` está protegido en tres capas: el middleware exige sesión antes de servir la página, `app/laboratorio/layout.tsx` vuelve a comprobarlo en el servidor, y las políticas RLS de Postgres son la barrera final si alguna de las anteriores fallara.

## 3. Rutas creadas

```text
/laboratorio                        Dashboard con estadísticas reales y actividad reciente
/laboratorio/pacientes              Listado, búsqueda y paginación
/laboratorio/pacientes/nuevo        Alta de paciente (formulario por secciones)
/laboratorio/pacientes/[id]         Ficha: datos, contacto, historial de muestras/solicitudes/informes
/laboratorio/pacientes/[id]/editar  Edición de paciente
/laboratorio/muestras               Listado de muestras
/laboratorio/muestras/nueva         Alta de muestra (código autogenerado)
/laboratorio/solicitudes            Listado con filtro por estado
/laboratorio/solicitudes/nueva      Alta de solicitud
/laboratorio/solicitudes/[id]       Detalle: análisis, registro de resultados, finalizar, generar informe
/laboratorio/resultados             Listado global de resultados con filtro por estado
/laboratorio/informes               Listado de informes
/laboratorio/informes/[id]          Detalle del informe, emitir/anular, descargar PDF
/laboratorio/informes/[id]/pdf      Descarga del PDF (Route Handler, Node runtime)
/laboratorio/configuracion          Identidad del laboratorio + logo/firma/sello (solo admin escribe)
```

Todas las rutas anteriores de las Fases 1-3 (auth, `/admin`, `/contenido`, `/buscar`, etc.) siguen funcionando sin cambios; el build de esta fase las incluye a todas sin errores.

## 4. Base de datos (tablas nuevas)

`patients` · `samples` · `lab_orders` · `lab_order_items` · `lab_results` · `lab_reports` · `lab_settings` (fila única) · `reference_ranges`.

- Todas con `created_at`/`updated_at`; `patients`, `samples`, `lab_orders`, `lab_results`, `lab_reports` además tienen `created_by`/`updated_by`.
- `reference_ranges` queda con la estructura pedida (sexo, edad mínima/máxima, rango mínimo/máximo, unidad, texto libre) y su propia RLS, pero **esta fase no construye un algoritmo clínico automático sobre ella** — se deja explícitamente preparada para una fase futura, tal como pide el encargo ("no implementar todavía algoritmos clínicos complejos"). Hoy el rango que se imprime en el informe es el texto que el usuario confirma manualmente al registrar el resultado (precargado desde `clinical_analyses.reference_range` cuando existe).
- `lab_results.flag` (bajo/normal/alto) se calcula con `computeFlag()` solo cuando el resultado es numérico y se proporcionan límites numéricos explícitos; nunca se infiere un rango que no exista.

## 5. Numeración atómica

Nunca se usa `SELECT MAX(...) + 1`. Se usan **secuencias de PostgreSQL** (`patients_code_seq`, `samples_code_seq`, `lab_orders_code_seq`, `lab_reports_code_seq`) envueltas en funciones `SECURITY DEFINER` (`next_patient_code()`, `next_sample_code()`, `next_order_code()`, `next_report_number()`), llamadas vía RPC desde las Server Actions. `nextval()` es atómico a nivel de Postgres, por lo que dos usuarios creando registros al mismo tiempo nunca reciben el mismo número.

Formato: `PAC-000001`, `MUE-000001`, `SOL-000001`, `LAB-000001`.

## 6. Resultados: cuatro tipos soportados

`components/lab/ResultEntry.tsx` adapta el formulario según `lab_order_items.result_type`:

- **Cuantitativo**: resultado + unidad + valor de referencia (texto).
- **Cualitativo**: selector (Positivo/Negativo/Reactivo/No reactivo/Presente/Ausente/Normal/Anormal).
- **Semicuantitativo**: selector (Negativo, +, ++, +++, ++++).
- **Descriptivo**: texto libre.

Todos permiten una "Observación del resultado" adicional. Un análisis puede validarse (`validado`) después de ingresado.

## 7. Informes y PDF

- "Generar informe" desde una solicitud crea la fila en `lab_reports` con numeración atómica y copia los datos del profesional responsable desde `lab_settings` (se pueden editar por informe si hace falta).
- El PDF se genera con `@react-pdf/renderer` (`components/lab/LabReportPDF.tsx`), renderizado en el servidor por `app/laboratorio/informes/[id]/pdf/route.tsx` (Route Handler en runtime Node, sin navegador headless). Incluye encabezado con logo, datos del paciente, datos de la muestra, tabla de resultados (examen/resultado/unidad/valores de referencia), observaciones, y bloque de firma + sello.
- Nombre de archivo: `LABDEX-000001.pdf` (a partir del número de informe `LAB-000001`).
- Logo, firma y sello se leen del bucket privado `lab-assets` mediante signed URLs generadas en el momento de renderizar el PDF.

## 8. Componentes principales creados

`PatientForm`, `SampleForm`, `OrderForm`, `AddAnalysisForm`, `ResultEntry`, `OrderActions`, `ReportActions`, `LabSettingsForm`, `LabAssetField`, `LabReportPDF`, `StatusBadge`, `LabSidebar`.

## 9. Manejo de errores

Todas las Server Actions (`lib/lab/actions.ts`, `lib/lab/resultsAndReports.ts`, `lib/lab/assets.ts`) capturan el código de error de Postgres y devuelven mensajes amigables (`"Ya existe un registro con estos datos."`, `"El registro relacionado no existe o fue eliminado."`) en vez del error técnico crudo; los detalles se registran solo en consola del servidor.

## 10. Cómo probar el flujo completo

1. Ve a `/laboratorio/pacientes/nuevo` y registra un paciente.
2. Desde su ficha, "Registrar muestra" y luego "Crear solicitud".
3. En el detalle de la solicitud, agrega uno o más análisis (del catálogo de `clinical_analyses` de la Fase 2), ingresa resultados, valida si corresponde, y pulsa "Finalizar solicitud".
4. Pulsa "Generar informe" → te lleva al informe recién creado.
5. En `/laboratorio/configuracion`, sube un logo/firma/sello (requiere una cuenta `admin`).
6. Vuelve al informe y pulsa "Descargar PDF": debe incluir la identidad del laboratorio configurada.
7. Prueba con una cuenta `user` que NO seas admin: puedes usar todo el módulo de laboratorio (crear pacientes, solicitudes, resultados), pero al entrar a `/laboratorio/configuracion` el formulario aparece bloqueado (`disabled`), y si se intentara forzar la escritura, la política RLS de `lab_settings` la rechazaría.
8. Cierra sesión e intenta entrar a cualquier `/laboratorio/*` por URL directa: debes ser redirigido a `/login`.

## 11. Qué queda preparado para la Fase 5 (LABDEX AI)

- Cada entidad clínica (paciente, muestra, solicitud, resultado, informe) tiene ID estable y relaciones claras, lista para que un futuro servicio de contexto se la entregue a Gemini bajo autorización explícita — nunca acceso automático a todos los pacientes.
- `reference_ranges` ya existe con la estructura necesaria para que una fase futura implemente interpretación automática por edad/sexo.
- El botón "Consultar LABDEX AI" ya está preparado (deshabilitado) en las fichas públicas de contenido desde la Fase 3.

---

# LABDEX — Fase 3: Interfaz pública + Enciclopedia de laboratorio (referencia)
